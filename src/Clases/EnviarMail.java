...
public class EnviarMail implements Runnable {
    ...
    public EnviarMail(String correo, String contraseña, JTextField destinatario, JTextField asunto, JTextField archivoAdjunto, JTextArea mensaje, String nombre, JLabel estado) {
        this.correo = correo;
    ...
    public void run() {
        if (rutaArchivo.equals("")) {
            ...
            //Clase encargada de almacenar las propiedades de conexión que va a
            //establecer con el servidor de correo saliente SMTP
            Properties props = new Properties();

            //Creación de conexión a servidores
            //El método put asigna las propiedades
            if(Login.servidor == 0){
            ...
            //Clase encargada de manejar la sesión de usuario
            ...
            try {
                ...
                for (int i = 0; i < vect.length; i++) {
                    //MimeMessage forma el mensaje que deseamos enviar
                    Message message = new MimeMessage(sesion);
                    message.setFrom(new InternetAddress(usuario));
                    message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(vect[i]));
                    message.setSubject(asu);
                    message.setText(msg);
                    //Clase transport define los parametros del protocolo de transporte
                    Transport.send(message);
                    JOptionPane.showMessageDialog(null, "Su mensaje fué enviado exitosamente\na los siguientes contactos: \n" + vect[i]);
                }
            ...
            try {
                ...
                for (int i = 0; i < vect.length; i++) {
                    ...
                    //Método connect se encarga de establecer conexión con el servidor
                    //introduciendo el nombre de usuario y contraseña
                    t.connect(correo, contraseña);
                    //Método sendmessage envia el mensaje que hemos creado a los destinatarios especificados
